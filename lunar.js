var LunarCalendar = (function() {
    ...
    function solarToLunar(year, month, day) {
        ...
        var lMonthName = monthNames[lMonth - 1];
        if (isLeap) {
            lMonthName = '闰' + lMonthName;
        }
        var lDayName = dayNames[lDay - 1];
    ...
    function getSolarTerms(year) {
        if (_solarTermsCache[year]) return _solarTermsCache[year];
        var terms = {};
        var baseTime = Date.UTC(1900, 0, 5, 18, 5, 0);
        var tropicalYear = 31556925974.7;
        for (var i = 0; i < 24; i++) {
            var offset = tropicalYear * (year - 1900) + sTermInfo[i] * 60000;
            var termMs = baseTime + offset + 8 * 3600000;
            var termDate = new Date(termMs);
            var mm = termDate.getUTCMonth() + 1;
            var dd = termDate.getUTCDate();
            var key = (mm < 10 ? '0' : '') + mm + '-' + (dd < 10 ? '0' : '') + dd;
            if (!terms[key]) {
                terms[key] = sTermNames[i];
            }
        }
        _solarTermsCache[year] = terms;
        return terms;
    }
    ...
    function getLunarFestival(lMonth, lDay, isLeap, lYear) {
        ...
        if (lMonth === 12) {
            var daysInMonth = monthDays(lYear, 12);
            if (lDay === daysInMonth) return '除夕';
        }
        return null;
    }
    ...
    return {
        ...
        getLunarFestival: getLunarFestival
    };
})();
